...
(function() {
    ...
    e = {
        init: function(b) {
            ...
            g = d.length; g > f; f++)
                c = d[f],
                h.push(e.initTable(c));
            return h
        },
        initTable: function(a) {
            var b, c, d, f, g, h;
            h = a.tHead;
            if ("true" !== a.getAttribute("data-sortable-initialized")) {
                for (a.setAttribute("data-sortable-initialized", "true"),
                d = a.querySelectorAll("th"),
                b = f = 0,
                g = d.length; g > f; b = ++f)
                    // Set up clicks modulo the table column length so that floating headers can also be sortable.
                    c = d[b],
                    "false" !== c.getAttribute("data-sortable") && e.setupClickableTH(a, c, b % h.rows[0].children.length, h.rows[0].children.length);
                return a
            }
        },
        setupClickableTH: function(a, d, f, headerLength) {
            var g, h, i, j, k, l;
            for (i = e.getColumnType(a, f),
            h = function(b) {
                ...
                p = this.parentNode.parentNode.querySelectorAll("th"),
                s = 0,
                w = p.length; w > s; s++) {
                    d = p[s],
                    d.setAttribute("data-sorted", "false"),
                    d.removeAttribute("data-sorted-direction");
                    // ALso show the sort visually for the headers on another row.
                    if (s % headerLength == f) {
                        d.setAttribute("data-sorted", "true");
                        d.setAttribute("data-sorted-direction", h);
                    }
                }
                if (this.setAttribute("data-sorted", "true"),
        ...
        getNodeValue: function(a) {
            var b;
            return a ? (b = a.getAttribute("data-value"),
            null !== b ? b : "undefined" != typeof a.innerText ? a.innerText.replace(g, "") : a.textContent.replace(g, "")) : ""
        },
    ...
    e.setupTypes([{
	  ...
	  name: 'grade',
	  defaultSortDirection: 'descending',
	  match: (a) => { return a.match(/[A-D][+-]?/) != null; },
	  compare: (a, b) => {
	  	// Convert each grade to a numeric value.
	  	const gradeMap = {
			'A+': 9,
			'A': 8,
			'A-': 7,
		...
		// Return a if a is larger, else b.
		const aValue = gradeMap[a];
		const bValue = gradeMap[b];
		if (aValue > bValue) {
			return -1;
